...
void multi_test(void){
  ...
  getFolderFile(file_name.c_str(), files);
  ...
  for(const auto & file:files){
    std::string dataFile = std::string(file_name) + file + ".bin";
    float *points_array;
    int in_num_points;
    in_num_points =
    Bin2Arrary(points_array, dataFile, config["LoadDim"].as<int>(),
    ...
    std::string boxes_dir_name = config["OutputFile"].as<std::string>();
    std::string boxes_file_name=boxes_dir_name+file+".txt";
    Boxes2Txt(out_detections, out_labels, out_scores, boxes_file_name);
    delete[] points_array;
    //limit detect numbers 
    // rate_count++;
    // if(rate_count>=100){
    //   break;
    // }
