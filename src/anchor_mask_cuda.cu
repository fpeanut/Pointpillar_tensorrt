...
__global__ void scan_xV2(int* g_odata, int* g_idata, int n) {
...
// modified prefix sum code from
// https://www.mimuw.edu.pl/~ps209291/kgkp/slides/scan.pdf
__global__ void scan_yV2(int* g_odata, int* g_idata, int n) {
  extern __shared__ int temp[];  // allocated on invocation
  int thid = threadIdx.x;
  int bid = blockIdx.x;
  int bdim = blockDim.x;
...
__global__ void make_anchor_mask_kernelV2(
...
void AnchorMaskCuda::DoAnchorMaskCuda(
  ...
  scan_xV2<<<num_inds_for_scan_, num_inds_for_scan_ / 2,
           num_inds_for_scan_ * sizeof(int)>>>(
      dev_cumsum_along_x, dev_sparse_pillar_map, num_inds_for_scan_);
  scan_yV2<<<num_inds_for_scan_, num_inds_for_scan_ / 2,
           num_inds_for_scan_ * sizeof(int)>>>(
  ...
  make_anchor_mask_kernelV2<<<num_blocks, num_threads_>>>(
