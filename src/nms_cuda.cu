...
__device__ inline float devIoUV2(float const *const a, float const *const b) {
...
__global__ void nms_kernelV2(const int n_boxes, const float nms_overlap_thresh,
  ...
  if (threadIdx.x < row_size) {
    ...
    for (int i = start; i < col_size; ++i) {
      if (devIoUV2(cur_box, block_boxes + i * num_box_corners) >
          nms_overlap_thresh) {
        t |= 1ULL << i;
      }
    }
...
NmsCudaV2::NmsCudaV2(const int num_threads, const int num_box_corners,
                 const float nms_overlap_threshold)
    : num_threads_(num_threads),
      num_box_corners_(num_box_corners),
      nms_overlap_threshold_(nms_overlap_threshold) {}

void NmsCudaV2::DoNmsCuda(const int host_filter_count,
  ...
  std::cout<<"host_filter_count:"<<host_filter_count<<std::endl;
  std::cout<<"col_blocks:"<<col_blocks<<std::endl;
  GPU_CHECK(
      cudaMalloc(&dev_mask, host_filter_count * col_blocks * sizeof(uint64_t)));

  nms_kernelV2<<<blocks, threads>>>(host_filter_count, nms_overlap_threshold_,
