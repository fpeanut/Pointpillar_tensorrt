...
__global__ void filter_kernelV2(
...
__global__ void sort_boxes_by_indexes_kernelV2(
...
PostprocessCudaV2::PostprocessCudaV2(const float float_min, const float float_max,
  ...
  nms_cuda_ptr_.reset(
      new NmsCudaV2(num_threads, num_box_corners, nms_overlap_threshold));
}

void PostprocessCudaV2::DoPostprocessCuda(
  ...
  std::cout<<"num_blocks_filter_kernel:"<<num_blocks_filter_kernel<<std::endl;
  filter_kernelV2<<<num_blocks_filter_kernel, num_threads_>>>(
  ...
  sort_boxes_by_indexes_kernelV2<<<num_blocks, num_threads_>>>(
