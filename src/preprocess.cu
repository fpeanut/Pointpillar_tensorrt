...
__global__ void make_pillar_index_kernel(
    ...
    const int num_point_feature,int* dev_sparse_pillar_map,const int num_inds_for_scan) {
  int x = blockIdx.x;
  int y = threadIdx.x;
  ...
  if (cur_pillar_id < max_pillars) {
    ...
    dev_sparse_pillar_map[y * num_inds_for_scan + x] = 1;//mask
...
__global__ void gather_point_feature_kernel(
  ...
  //   dev_pillar_point_feature[ith_pillar * max_num_points_per_pillar *
  //                                 num_point_feature +
  //                             ith_point * num_point_feature + 4];

  // dev_pfe_gather_feature_[ith_pillar * max_num_points_per_pillar *
  //                             num_gather_feature +
  //                         ith_point * num_gather_feature + 4] = 0.0f;
...
PreprocessPointsCuda::PreprocessPointsCuda(
    ...
    const float max_z_range,const int num_inds_for_scan)
...
void PreprocessPointsCuda::DoPreprocessPointsCuda(
    ...
    float* dev_pfe_gather_feature,int* dev_sparse_pillar_map) {
  ...
  make_pillar_index_kernel<<<grid_x_size_, grid_y_size_>>>(
      ...
      num_point_feature_,dev_sparse_pillar_map,num_inds_for_scan_);
