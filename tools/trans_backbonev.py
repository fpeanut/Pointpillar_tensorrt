...
# from ../builder import (BACKBONES, HEADS, NECKS)
...
class Backbone(nn.Module):
    ...
    def forward(self, input):
        ...
        # waymo
        x = input[:64*468*468]
        x = x.reshape(-1, 64, 468, 468)
        # anchors = input[64*468*468:]
        ...
        cls_score = x[0][0].detach()
        bbox_pred = x[1][0].detach()
        dir_pred  = x[2][0].detach()

        cls_score=cls_score.permute(0,2,3,1)
        bbox_pred=bbox_pred.permute(0,2,3,1)
        dir_pred=dir_pred.permute(0,2,3,1)
...
def main():
    ...
    anchors = np.load('tools/onnx_tools/pointpillars/waymod_v2/anchors.npy')
    anchors = torch.from_numpy(anchors).cuda("cuda:0")
    print("-----------------ready to export onnx --------------------------")
    rfeature_map = feature_map.reshape(-1)
    ranchors = anchors.reshape(-1)
    # ranchors = ranchors.float().to(torch.float32)
    print(rfeature_map.dtype)
    print(ranchors.dtype)
    # input = torch.cat((rfeature_map, ranchors))
    input=rfeature_map

    # export to onnx
    export_onnx_file = './tools/onnx_tools/pointpillars/model/backbone.onnx'
