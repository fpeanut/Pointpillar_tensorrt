...
def build_vfe_model(config, checkpoint=None, device='cuda:0'):
    ...
    config.model.pretrained = None #新加的属性设置为none
    convert_SyncBN(config.model) #将norm_cfg里naiveSyncBN1d换成BN
...
def main():
    ...
    dummy_input = torch.ones(config.model['pts_voxel_layer']['max_voxels'][1],
                             config.model['pts_voxel_layer']['max_num_points'],
                             pts_voxel_encoder.in_channels).cuda()
    export_onnx_file = './tools/onnx_tools/pointpillars/model/vfe.onnx'
